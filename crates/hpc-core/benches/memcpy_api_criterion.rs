    ...
    for &nbytes in SIZES {
        ...
        group.bench_function(format!("vector_add_{}", nbytes), |bench| {
            bench.iter(|| {
                ...
                let buffer_a = ctx.create_empty_buffer::<u8>(nbytes).unwrap()
                    .write_block(&queue, black_box(&a)).unwrap();
                
                let buffer_b = ctx.create_empty_buffer::<u8>(nbytes).unwrap()
                    .write_block(&queue, black_box(&b)).unwrap();
                
                let buffer_result = ctx.create_empty_buffer::<u8>(nbytes).unwrap()
                    .write_block(&queue, black_box(&result)).unwrap();
                ...
                // 3. Kernel ausführen
                let (inflight_buffer, event) = buffer_result.enqueue_kernel(&queue, &kernel, nbytes).unwrap();

                // 4. Warten und Ergebnis zurücklesen
                let result_buffer = event.wait(inflight_buffer);
                result_buffer.read_blocking(&queue, black_box(&mut result)).unwrap();
            });
        });
    }
...
fn raw_full_bench(c: &mut Criterion) {
    ...
    // OpenCL Setup (raw)
    let device_ids = opencl3::device::get_all_devices(opencl3::device::CL_DEVICE_TYPE_GPU).expect("Kein GPU gefunden");
    let ctx = opencl3::context::Context::from_devices(&device_ids, &[], None, ptr::null_mut()).unwrap();
    ...
    for &nbytes in SIZES {
        ...
        group.bench_function(format!("vector_add_{}", nbytes), |bench| {
            bench.iter(|| {
                ...
                let _event = queue.enqueue_nd_range_kernel(
