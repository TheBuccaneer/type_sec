...
fn memcpy_bench(c: &mut Criterion) {
    ...
    for &nbytes in SIZES {
        ...
        let buf = create_buffer::<u8>(&ctx, src.len()).unwrap();
        let mut buf = buf.enqueue_write(&queue, &src).unwrap();

        group.bench_function(format!("copy_bytes_{}", nbytes), |b| {
            b.iter(|| {
                buf.benchmark_overwrite_non_blocking(&queue, black_box(&src))
                    .unwrap();
...
fn memcpy_opencl3_bench(c: &mut Criterion) {
    ...
    for &nbytes in SIZES {
        ...
        group.bench_function(format!("copy_only_{}", nbytes), |b| {
            b.iter(|| {
                queue
                    .enqueue_write_buffer(&mut buf, CL_NON_BLOCKING, 0, black_box(&src), &[])
                    .unwrap();
                queue
