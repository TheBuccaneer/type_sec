...
fn main() -> Result<(), ClError> {
    ...
    for _ in 0..num_buffers {
        gpu_buffers.push(Buffer::<f32>::create(
            &context,
            CL_MEM_READ_WRITE,
            chunk_size,
            ptr::null_mut(),
    ...
    for iter in 0..5 {
        let mut big_buffer =
            Buffer::<f32>::create(&context, CL_MEM_READ_WRITE, total_floats, ptr::null_mut())?;
        let start = Instant::now();
        let evt = queue.enqueue_write_buffer(
            &mut big_buffer,
            CL_NON_BLOCKING,
            0,
            cast_slice(&host_data),
            &[],
        )?;
        evt.wait()?;
        let elapsed = start.elapsed().as_secs_f64();
    ...
    for iter in 0..5 {
        ...
        let evt = queue.enqueue_read_buffer(
            ...
            CL_NON_BLOCKING,
            0,
            cast_slice_mut(&mut result_data),
            &[],
        )?;
        evt.wait()?;
        let elapsed = start.elapsed().as_secs_f64();
    ...
    println!("\n=== BASIC RAW RESULTS ===");
    println!(
        "Data size: {:.2} GB ({} buffers)",
        total_size_gb, num_buffers
    );
    println!(
        "Multi-buffer H2D: {:.3} ms, {:.2} GB/s",
        h2d_avg * 1000.0,
        h2d_bandwidth
    );
    println!(
        "Multi-buffer D2H: {:.3} ms, {:.2} GB/s",
        d2h_avg * 1000.0,
        d2h_bandwidth
    );
    println!(
        "Pure single H2D:  {:.3} ms, {:.2} GB/s",
        pure_h2d_avg * 1000.0,
        pure_h2d_bandwidth
    );
    println!(
        "Pure single D2H:  {:.3} ms, {:.2} GB/s",
        pure_d2h_avg * 1000.0,
        pure_d2h_bandwidth
    );
    ...
    println!("\n=== RAW EFFICIENCY ===");
    println!(
        "Pure raw efficiency: H2D {:.1}%, D2H {:.1}%",
    ...
    println!("\n=== OVERHEAD ANALYSIS ===");
    println!(
        "Multi-buffer overhead: H2D {:.1}%, D2H {:.1}%",
