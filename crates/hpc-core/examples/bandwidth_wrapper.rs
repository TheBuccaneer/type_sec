...
use hpc_core::{TracingScope, flush_csv};
...
fn main() -> Result<(), ClError> {
    ...
    for iter in 0..iterations {
        ...
        for (i, ready_buf) in iter_ready_buffers.into_iter().enumerate() {
            ...
            let (in_flight, guard) =
                ready_buf.enqueue_read(&queue, cast_slice_mut(chunk_result))?;
    ...
    for iter in 0..5 {
        ...
        let start = Instant::now();
        let (in_flight, guard) =
            ready_for_d2h.enqueue_read(&queue, cast_slice_mut(&mut result_data))?;
        drop(guard); // Warten
        drop(in_flight);
        let elapsed = start.elapsed().as_secs_f64();
    ...
    println!("\n=== WRAPPER EFFICIENCY ===");
    println!(
        "Pure wrapper efficiency: H2D {:.1}%, D2H {:.1}%",
