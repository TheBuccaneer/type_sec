...
#[command(
    name = "metrics_tail",
    about = "Tailt die letzten JSONL-Zeilen aus run.jsonl"
)]
struct Opt {
...
fn main() -> Result<()> {
    ...
    for l in buf {
        let v: Value = serde_json::from_str(&l).with_context(|| "invalid JSON line")?;
