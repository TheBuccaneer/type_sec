...
use std::{fs, thread, time::Duration, sync::Arc};
...
fn concurrent_appends_are_consistent() {
    let base = std::env::temp_dir()
        .join(format!("hpc-core-test-concurrency-{}", std::process::id()));
    let threads = 6usize;
    let per = 50usize;
    ...
    handles.push(thread::spawn(move || {
        for i in 0..per {
            if (t + i) % 7 == 0 { thread::sleep(Duration::from_millis(1)); }
            // Fix 1: n als usize
            let _ = log_run_to(&RunLog { example: "vec_add", n: i as usize }, &*base).unwrap();
        }
    }));
}
...
for l in &lines {
    ...
    let rid = v.get("run_id").and_then(|x| x.as_str()).expect("run_id present");
