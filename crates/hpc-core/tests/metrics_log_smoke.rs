...
fn jsonl_writer_appends_one_line() {
    ...
    // 1. Write
    let p1 = log_run_to(&RunLog { example: "vec_add", n: 256 }, &base).expect("first write failed");
    // 2. Write (Append, gleicher Pfad)
    let p2 = log_run_to(&RunLog { example: "vec_add", n: 512 }, &base).expect("second write failed");
    assert_eq!(p1, p2, "wrote to different files");

    let mut s = String::new();
    std::fs::File::open(&p1).unwrap().read_to_string(&mut s).unwrap();
    let lines: Vec<_> = s.lines().map(|l| l.trim()).filter(|l| !l.is_empty()).collect();
    assert_eq!(lines.len(), 2, "expected exactly 2 JSONL lines after two writes");

    for (i, line) in lines.iter().enumerate() {
        let v: serde_json::Value = serde_json::from_str(line).expect(&format!("line {} not valid JSON", i));
...
fn jsonl_line_has_counters() {
    ...
    let p = log_run_to(&RunLog { example: "vec_add", n: 1 }, &base).expect("write failed");
